...
const ReceituarioView = ({
  receituarios,
  isLoading,
  openReceituarioModal,
  contatos,
  recursos,
}) => {
  const getNome = (id, lista) =>
    lista.find((item) => item.id === id)?.nome || "N/A";
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      {isLoading ? (
        <div className="text-center p-12">
          <LucideIcon
            name="FileText"
            className="w-12 h-12 animate-spin text-green-500 mx-auto"
          />
              ...
              {receituarios.map((r) => (
                <tr key={r.id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    {getNome(r.cliente, contatos)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {r.cultura}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {getNome(r.produto, recursos)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
